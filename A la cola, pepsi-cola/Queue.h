...
public:
	...
	*/
	...
	/**
	 Une la cola del parametro a partir de la pos T
	 */
	void colar(const T& p, Queue<T>& colegas) {
		if (!colegas.empty()) {	// Comprobar que la lista de colegas no esta vacia
			Nodo* actual = _prim;
			while (actual != nullptr && actual->_elem != p) {	//Buscar el elemento
				actual = actual->_sig;
			}
			if (actual != nullptr) {	//si se ha encontrado la pos continuar
			    if (actual== _ult) //si el pringao es el último actualizamos el último de la cola
			        _ult=colegas._ult;
				Nodo* aux = actual->_sig;
				actual->_sig = colegas._prim;
				colegas._ult->_sig = aux;
				_numElems += colegas._numElems;
				//hacemos que colegas esté vacía
